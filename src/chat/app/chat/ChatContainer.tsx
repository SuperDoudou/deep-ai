...
class MessageItem {
    ...
    public addMessageChunk = (messgeChunk: string) => {
        this.message += messgeChunk
    }
    public setReasoningContent = (reasoningContent: string) => {
        this.reasoningContent = reasoningContent;
    };
    public addReasoningContextChunk = (reasoningContentChunk: string) => {
        this.reasoningContent += reasoningContentChunk
    }
...
function ChatContainer() {
    ...
    const onInputMessage = (userPrompt: string, contextAreaInfo: ContextAreaInfo, promptTemplate: string) => {
        ...
        let prompt = promptTemplate.replace("${user_prompt}", userPrompt)
        prompt = prompt.replace("${fileText}", contextAreaInfo.fileText)
        prompt = prompt.replace("${fileName}", contextAreaInfo.fileName)
        const p = ExtensionServiceInterface.getInstance().chat({ prompt: prompt })
        p.then(async stream => {
            for await (const chunk of stream) {
                console.log(`get chunk ${chunk.content}`)
                if (answerMessageItem.status === "stop") {
                    return
                }

                answerMessageItem.addMessageChunk(chunk.content);
                answerMessageItem.addReasoningContextChunk(chunk.reasoningContent);
                setCount(count => count + 1)
            }
