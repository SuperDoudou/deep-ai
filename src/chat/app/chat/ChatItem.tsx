...
interface MessageBlock {
    id: number;
    type: "text" | "code"; // 消息类型，如 text, code
...
function ChatItem({ id, name, isRobot, avatar, message, reasoning, userPrompt, contextAreaInfo }: ChatItemProps) {
    ...
    let parseMessage = (message: string): MessageBlock[] => {
        ...
        message.split("```").forEach((block, index) => {
            if (index % 2 === 0) {
                tempResult.push({ id: index, type: "text", content: block });
            } else {
                let [language, ...content] = block.split("\n");
                if (!language || language.trim() === "") {
                    language = "text";
                }
                tempResult.push({ id: index, type: "code", language: language.trim(), content: content.join("\n") });
            }
        });
        return tempResult;
    }
    ...
    const clickApplyCode = (code: string) => {
        ...
        let prompt = promptTemplate.replace("${userPrompt}", userPrompt)
            ...
            <div className="chat-item-content">
                ...
                {messageBlocks.map((block, index) => {
                    if (block.type === "text" && block.content !== "") {
                        return <p className="chat-item-content-text" key={index}>{block.content}</p>;
                    }
                    else if (block.type === "code") {
                        return (
                            <div className="chat-item-content-code" key={index}>
                                <div className="chat-item-language-block">
                                    ...
                                    <div className="chat-item-language-right">
                                        ...
                                        {hasInsert && !hasAccept && (
                                            <img className="chat-item-language-button"
                                                src={Utils.svgToDataURL(acceptIcon)}
                                                title='接受代码'
                                                onClick={() => {
                                                    setHasAccept(true)
